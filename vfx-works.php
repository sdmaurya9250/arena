...
<h1 class="wow fadeInDownBig">VFX</h1>
<div class="row mediawork">
<?php
                              $counter = 0;
                              $courses = $db->getRows("SELECT * from students_works_vfx order by vfx_id desc");
                              if( count($courses) > 0){
                                foreach($courses as $data){ 
								  $design_id = $data["vfx_id"];
                                  $vfx_name = $data["vfx_name"];
                                  $video = $data["vfx_file"];  
                                  $counter++;
