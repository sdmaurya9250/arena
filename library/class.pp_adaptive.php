...
class PP_ADAPTIVE
{
	...
	*/
						...
						$memo, $pin, $preapprovalKey, $reverseAllParallelPaymentsOnError, $senderEmail, $trackingId )
	{
		...
		*/
		...
		if (0 != count($receiverAmountArray))
		{
			...
			{
				if ("" != $value) $arynvp["receiverList.receiver(" . $key . ").amount"]=$value;
			}
		}
		...
		if (0 != count($receiverEmailArray))
		{
			...
			{
				if ("" != $value) $arynvp["receiverList.receiver(" . $key . ").email"]=$value;
			}
		}
		...
		if (0 != count($receiverPrimaryArray))
		{
			...
			{
				if ("" != $value) $arynvp["receiverList.receiver(" . $key . ").primary"]=$value;
			}
		}
		...
		if (0 != count($receiverInvoiceIdArray))
		{
			...
			{
				if ("" != $value) $arynvp["receiverList.receiver(" . $key . ").invoiceId"]=$value;
			}
		}
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------	
	*/
	public function CallPreapproval($returnUrl, $cancelUrl, $currencyCode, $startingDate, $endingDate, 
									$maxTotalAmountOfAllPayments, $senderEmail, $maxNumberOfPayments, $paymentPeriod, 
									$dateOfMonth, $dayOfWeek, $maxAmountPerPayment, $maxNumberOfPaymentsPerPeriod, $pinType,
									$ipnNotificationUrl )
	{
