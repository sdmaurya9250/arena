  ...
  if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['image']['name'][$i])) {   
      ...
      if (in_array($ext, array('jpeg', 'jpg', 'gif', 'png','webp'))) {
          ...
          if (move_uploaded_file($_FILES['image']['tmp_name'][$i], '../images/student_work_graphics/'.$newfile)){
            $filename = 'images/student_work_graphics/'.$newfile;                
          }
      } 
      else {
          $filename = "";
      ...
      $create = array(
                          ...
                          'graphics_name' =>  $_POST["title"],
                          
                                                 
                         
                      );
                     
      $rest = $db->insertAry('students_works_graphics',$create);
                    }
                  }
      ...
      if(!is_null($rest)){
        ...
        echo "<script>window.location.href='students_works_graphics.php';</script>";
      }else{ 
...
if (isset($_POST['update'])) {
    ...
    if (isset($_FILES['image']['name'][$i])) {   
      ...
      $create = array(
                          ...
                          'graphics_file' => $filename, 
                          'graphics_name' =>  $_POST["graphics_name"],
                          
                                               
                         
                      );
    //   $rest = $db->insertAry('testimonials',$create);
      $rest = $db->updateAry("students_works_graphics", $create, "where graphics_id='".$_POST['graphics_id']."'");
...
<div id="page-wrapper">
  ...
  <div class="row panel panel-default mgt-20"  >
    <div class="panel-heading">
      <h1 class="white-text page-heading">Students Works Graphics</h1>
    </div>
        ...
        <div class="panel-default">
         ...
         ?>
                        <tr class="odd gradeX">
                          ...
                          <td><?php echo $graphics_name ?></td>
                          <td><img src='<?=$image?>' style='max-width: 100px;'/></td>
                          
                                                                             
                          
                          <td class="text-center ac_user_edit">
                                    <button class="123 btn btn-primary btn-circle  GraphicsEditStatus" style="background: #2b3252;" data-toggle="modal" data-target="#myModal" name="edit" data-id="<?php echo $id; ?>" data-myvalue="<?php echo $id; ?>" ><i class="fa fa-pencil-alt"></i>
                                  </button>
                            </td>
                          <td class="text-center ac_course_delete">
                            <button type="button" id="graphics_delete" onclick="DeleteConfirmation(this,'students_works_graphics')" name="graphics_delete" value="<?php  echo $id ?>" class="btn btn-danger btn-circle"><i class="fa fa-times"></i>
                            </button>
                          </td>
                        </tr>
...
$(document).on("click", ".GraphicsEditStatus", function(event) {
		event.preventDefault();
		var value = $(this).attr('data-id');
		$.ajax({
				url: 'ajax.php',
				type: 'POST',
				data: 'action=GraphicsEdit&id=' + value,
				dataType: 'html'
			})
			.done(function(data) {
				// console.log(data);  
              ...
              Okay: {
                ...
                action: function(){
                  $.ajax({
                    type: 'post',
                    url: 'ajax.php',
                    data: 'action=Graphics_delete&Id='+id+'&graphics_delete='+tablename,
                    dataType: "json",
