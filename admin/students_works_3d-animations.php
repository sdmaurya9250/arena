...
$embedTexts = preg_replace("/\s*[a-zA-Z\/\/:\.]*youtube.com\/watch\?v=([a-zA-Z0-9\-_]+)([a-zA-Z0-9\/\*\-\_\?\&\;\%\=\.]*)/i","<iframe width=\"240\" height=\"240\" src=\"//www.youtube.com/embed/$1\" frameborder=\"0\" allowfullscreen></iframe>",$texts);
    ...
    $sql="SELECT * FROM students_works_3d_animations";
    $result = $db->query($sql);
    while($row = $result->fetch_assoc()) 
    $video = $row['animations_file'];

$video_title = $_POST["video_title"];

 $sql = "INSERT INTO `students_works_3d_animations`(`animations_file`,`animations_name`)VALUES ('$embedTexts','$video_title')";
        if ($db->query($sql) === TRUE) 
        {
          echo "<script>alert('Added Successfully')</script>";
          echo "<script>window.location.href='students_works_3d-animations.php';</script>";
        } 
        else 
        {
...
<div id="page-wrapper">
  ...
  <div class="row panel panel-default mgt-20"  >
    <div class="panel-heading">
      <h1 class="white-text page-heading">Students Works 3D Animations</h1>
    </div>
        ...
        <div class="panel-default">
              ...
              <table width="100%" class="table table-bordered table-striped table-hover" id="dataTables-example">
                ...
                <tbody>
                  <?php
                        ...
                        $counter = 0;
                        $students_works = $db->getRows("SELECT * from students_works_3d_animations order by animations_id desc");
                        if(count($students_works) > 0){
                          foreach($students_works as $data){ 
                            $animations_id = $data["animations_id"];
                            $animations_file = $data["animations_file"];
         ...
         ?>
                        <tr class="odd gradeX">
                          ...
                          <td><?php echo $animations_file ?></td>
                          <td><?php echo $animations_name ?></td>
                       
                                                                             
                          
                         
                          <td class="text-center ac_course_delete">
                            <button type="button" id="students_works_delete" onclick="DeleteConfirmation(this,'students_works_3d_animations')" name="students_works_delete" value="<?php  echo $animations_id ?>" class="btn btn-danger btn-circle"><i class="fa fa-times"></i>
                            </button>
                          </td>
                        </tr>
              ...
              Okay: {
                ...
                action: function(){
                  $.ajax({
                    type: 'post',
                    url: 'ajax.php',
                    data: 'action=Students_3Dworksdelete&Id='+id+'&students_works_delete='+tablename,
                    dataType: "json",
