...
$embedTexts = preg_replace("/\s*[a-zA-Z\/\/:\.]*youtube.com\/watch\?v=([a-zA-Z0-9\-_]+)([a-zA-Z0-9\/\*\-\_\?\&\;\%\=\.]*)/i","<iframe width=\"240\" height=\"240\" src=\"//www.youtube.com/embed/$1\" frameborder=\"0\" allowfullscreen></iframe>",$texts);
    ...
    $sql="SELECT * FROM students_works_mix_media";
    $result = $db->query($sql);
    while($row = $result->fetch_assoc()) 
    $video = $row['media_file'];

$video_title = $_POST["video_title"];

 $sql = "INSERT INTO `students_works_mix_media`(`media_file`,`media_name`)VALUES ('$embedTexts','$video_title')";
        if ($db->query($sql) === TRUE) 
        {
          echo "<script>alert('Added Successfully')</script>";
          echo "<script>window.location.href='students_works_mix-media.php';</script>";
        } 
        else 
        {
...
<div id="page-wrapper">
  ...
  <div class="row panel panel-default mgt-20"  >
    <div class="panel-heading">
      <h1 class="white-text page-heading">Students Works MIX Media</h1>
    </div>
        ...
        <div class="panel-default">
              ...
              <table width="100%" class="table table-bordered table-striped table-hover" id="dataTables-example">
                ...
                <tbody>
                  <?php
                        ...
                        $counter = 0;
                        $students_works = $db->getRows("SELECT * from students_works_mix_media order by media_id desc");
         ...
         ?>
                        <tr class="odd gradeX">
                          ...
                          <td><?php echo $media_name ?></td>
                       
                                                                             
                          
                         
                          <td class="text-center ac_course_delete">
                            <button type="button" id="media_delete" onclick="DeleteConfirmation(this,'students_works_mix_media')" name="media_delete" value="<?php  echo $media_id ?>" class="btn btn-danger btn-circle"><i class="fa fa-times"></i>
                            </button>
                          </td>
                        </tr>
              ...
              Okay: {
                ...
                action: function(){
                  $.ajax({
                    type: 'post',
                    url: 'ajax.php',
                    data: 'action=Media_delete&Id='+id+'&media_delete='+tablename,
                    dataType: "json",
