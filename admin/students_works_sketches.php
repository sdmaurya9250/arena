  ...
  if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['image']['name'][$i])) {   
      ...
      if (in_array($ext, array('jpeg', 'jpg', 'gif', 'png','webp'))) {
          ...
          if (move_uploaded_file($_FILES['image']['tmp_name'][$i], '../images/student_work_sketches/'.$newfile)){
            $filename = 'images/student_work_sketches/'.$newfile;                
          }
      } 
      else {
          $filename = "";
      ...
      $create = array(
                          ...
                          'sketches_file' => $filename, 
                          'sketches_name' =>  $_POST["title"],
                          
                                                 
                         
                      );
      $rest = $db->insertAry('students_works_sketches',$create);
                    }
                  }
      ...
      if(!is_null($rest)){
        ...
        echo "<script>window.location.href='students_works_sketches.php';</script>";
      }else{ 
...
if (isset($_POST['update'])) {
    ...
    if (isset($_FILES['image']['name'][$i])) {   
      ...
      $create = array(
                          ...
                          'sketches_file' => $filename, 
                          'sketches_name' =>  $_POST["sketches_name"],
                          
                                               
                         
                      );
    //   $rest = $db->insertAry('testimonials',$create);
      $rest = $db->updateAry("students_works_sketches", $create, "where sketches_id='".$_POST['sketches_id']."'");
...
<div id="page-wrapper">
  ...
  <div class="row panel panel-default mgt-20"  >
    <div class="panel-heading">
      <h1 class="white-text page-heading">Students Works Sketches</h1>
    </div>
        ...
        <div class="panel-default">
              ...
              <table width="100%" class="table table-bordered table-striped table-hover" id="dataTables-example">
                ...
                <tbody>
                  <?php
                        ...
                        $counter = 0;
                        $courses = $db->getRows("SELECT * from students_works_sketches order by sketches_id desc ");
                        if( count($courses) > 0){
                          foreach($courses as $data){ 
                            $id = $data["sketches_id"];
                            $sketches_name = $data["sketches_name"];
                            $image = "../".$data["sketches_file"];
         ...
         ?>
                        <tr class="odd gradeX">
                          ...
                          <td><?php echo $sketches_name ?></td>
                          <td><img src='<?=$image?>' style='max-width: 100px;'/></td>
                          
                                                                             
                          
                          <td class="text-center ac_user_edit">
                                    <button class="123 btn btn-primary btn-circle SketchesEditStatus" style="background: #2b3252;" data-toggle="modal" data-target="#myModal" name="edit" data-id="<?php echo $id; ?>" data-myvalue="<?php echo $id; ?>" ><i class="fa fa-pencil-alt"></i>
                                  </button>
                            </td>
                          <td class="text-center ac_course_delete">
                            <button type="button" id="sketches_delete" onclick="DeleteConfirmation(this,'students_works_sketches')" name="sketches_delete" value="<?php  echo $id ?>" class="btn btn-danger btn-circle"><i class="fa fa-times"></i>
                            </button>
                          </td>
                        </tr>
...
$(document).on("click", ".SketchesEditStatus", function(event) {
		event.preventDefault();
		var value = $(this).attr('data-id');
		$.ajax({
				url: 'ajax.php',
				type: 'POST',
				data: 'action=SketchesEdit&id=' + value,
				dataType: 'html'
			})
			.done(function(data) {
				// console.log(data);  
              ...
              Okay: {
                ...
                action: function(){
                  $.ajax({
                    type: 'post',
                    url: 'ajax.php',
                    data: 'action=Sketches_delete&Id='+id+'&sketches_delete='+tablename,
                    dataType: "json",
