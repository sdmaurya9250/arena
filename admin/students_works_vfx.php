...
$embedTexts = preg_replace("/\s*[a-zA-Z\/\/:\.]*youtube.com\/watch\?v=([a-zA-Z0-9\-_]+)([a-zA-Z0-9\/\*\-\_\?\&\;\%\=\.]*)/i","<iframe width=\"240\" height=\"240\" src=\"//www.youtube.com/embed/$1\" frameborder=\"0\" allowfullscreen></iframe>",$texts);
    ...
    $sql="SELECT * FROM students_works_vfx";
    $result = $db->query($sql);
...
$video_title = $_POST["video_title"];
 ...
 $sql = "INSERT INTO `students_works_vfx`(`vfx_file`,`vfx_name`)VALUES ('$embedTexts','$video_title')";
        if ($db->query($sql) === TRUE) 
        {
          echo "<script>alert('Added Successfully')</script>";
          echo "<script>window.location.href='students_works_vfx.php';</script>";
        } 
        else 
        {
...
<div id="page-wrapper">
  ...
  <div class="row panel panel-default mgt-20"  >
    <div class="panel-heading">
      <h1 class="white-text page-heading">Students Works VFX</h1>
    </div>
        ...
        <div class="panel-default">
              ...
              <table width="100%" class="table table-bordered table-striped table-hover" id="dataTables-example">
                ...
                <tbody>
                  <?php
                        ...
                        $counter = 0;
                        $students_works = $db->getRows("SELECT * from students_works_vfx order by vfx_id desc");
                        if(count($students_works) > 0){
                          foreach($students_works as $data){ 
                            $vfx_id = $data["vfx_id"];
                            $vfx_file = $data["vfx_file"];
         ...
         ?>
                        <tr class="odd gradeX">
                          ...
                          <td><?php echo $vfx_file ?></td>
                          <td><?php echo $vfx_name ?></td>
                       
                                                                             
                          
                         
                          <td class="text-center ac_course_delete">
                            <button type="button" id="vfx_delete" onclick="DeleteConfirmation(this,'students_works_vfx')" name="vfx_delete" value="<?php  echo $vfx_id ?>" class="btn btn-danger btn-circle"><i class="fa fa-times"></i>
                            </button>
                          </td>
                        </tr>
              ...
              Okay: {
                ...
                action: function(){
                  $.ajax({
                    type: 'post',
                    url: 'ajax.php',
                    data: 'action=VFX_delete&Id='+id+'&vfx_delete='+tablename,
                    dataType: "json",
